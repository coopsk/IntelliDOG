...
//			GameWindow gw =  new GameWindow();
	...
	public void makeSeed()
	{
		...
		gm.setGamePolicy(OptimizationManager.GP_BEST_OF_FIVE);
		gm.begin(0,300, 0.0f);
		gm.dumpAll("/resources/genetic/ReferenceSeed");
	...
	public void testMutation()
	{
		...
		for(int i = 0; i<11; i++)
		{
			...
			gm.setCPUPolicy(OptimizationManager.CPU_MULTIPLE);
			gm.setGamePolicy(OptimizationManager.GP_BEST_OF_NINE);
			gm.begin(500, "/resources/genetic/ReferenceSeed", mutation);
			gm.getBest(); 
			gm.dumpAll(temp);
	...
	public void testPolicy()
	{
		...
		for(int i = 0; i<6; i++)
		{
			...
			gm.begin(500, "/resources/genetic/ReferenceSeed", 0.3f);
			gm.getBest(); 
			gm.dumpAll(output);
			paths[i] = output + "/"+ "rank0001.properties";
		}
	...
	public void testGenerations()
	{
		...
		for(int i = 0; i<10; i++)
		{
			...
			gm = new GeneticManager(); 
			gm.setCPUPolicy(OptimizationManager.CPU_MULTIPLE);
			gm.begin(population, "/resources/genetic/ReferenceSeed", 0.3f);
			gm.getBest(); 
			gm.dumpAll(output);
			paths[i] = output + "/"+ "rank0001.properties";
			population+=100; 
	...
	public void testDeepMutation()
	{
		...
		for(int i = 0; i<3; i++)
		{
			...
			gm.setGamePolicy(OptimizationManager.GP_BEST_OF_NINE);
			gm.begin(500, "/resources/genetic/ReferenceSeed", 0.7f);
			gm.getBest(); 
			gm.dumpAll(temp);
			paths[i] = temp + "/"+ "rank0001.properties";
			deepmutation++;
...
//		}
	...
	public void testSelectionPolicies()
	{
		...
		for(int i = 1; i<3; i++)
		{
			...
			gm.begin(500, "/resources/genetic/ReferenceSeed", 0.5f);
			gm.getBest(); 
			gm.dumpAll(output);
			paths[i] = output + "/"+ "rank0001.properties";
		}
