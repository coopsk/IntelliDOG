...
//		System.out.println(list.get(9).getGames_won_in_this_generation());
	...
	public void testPropertiesHandling()
	{
		List<EvaluatorWrapper> ew = new LinkedList<EvaluatorWrapper>();
		for(int i = 0; i<5; i++)
			ew.add(gm.makeRandomWrapper(gm.loadSeed("/genetic/RandomSeedGenetic.properties")));
		gm.setWrappers(ew);
		gm.dumpAll("testseed");
...
//		DebugMsg.getInstance().addItemForWhiteList(new GameRunner());
		...
		for(int i = 0; i<12; i++)
		{
			EvaluatorWrapper ew = gm.makeRandomWrapper(gm.loadSeed("/genetic/RandomSeedGenetic.properties"));
			ews.add(ew);
